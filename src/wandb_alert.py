...
def get_config():
    _config = {
        "wandb-project": "wandb-practice",
        "wandb-run-name": "alert example",
...
def train_one_epoch(epoch, device, model, optimizer, loss_function, dataloader):
    ...
    wandb.alert(
        title=f"Result of train {epoch} epoch",
        text=f"{epoch} epoch loss : {running_loss:.6f}",
...
def main():
    ...
    wandb.run.name = config['wandb-run-name']
